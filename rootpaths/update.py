...
class DatabaseManager:
    ...
    def connect(self, old_db_path, new_db_path):
        """
        Eski ve yeni veritabanına bağlanır. Eğer bağlantı sağlanamazsa uygun bir hata mesajı verir.
        """
        if not os.path.exists(old_db_path):
            logger.error(f"Eski veritabanı dosyası mevcut değil: {old_db_path}")
            return False

        if not os.path.exists(new_db_path):
            logger.error(f"Yeni veritabanı dosyası mevcut değil: {new_db_path}")
            return False

        try:
            self.old_conn = sqlite3.connect(old_db_path)
            logger.info(f"Eski veritabanına ({old_db_path}) başarıyla bağlanıldı.")
        except sqlite3.Error as e:
        ...
        try:
            ...
            logger.info(f"Yeni veritabanına ({new_db_path}) başarıyla bağlanıldı.")
        except sqlite3.Error as e:
    ...
    def update_old_databases(self, old_db_path, new_db_path):
        ...
        """
        if self.connect(old_db_path, new_db_path):
