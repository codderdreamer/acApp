                ...
                if self.application.ocppActive:
                    if self.application.settings.configuration.MinimumStatusDuration:
                        if time.time() - time_start > int(self.application.settings.configuration.MinimumStatusDuration):
                            asyncio.run_coroutine_threadsafe(self.application.chargePoint.send_status_notification(connector_id=1,error_code=self.application.chargePoint.error_code,status=self.application.chargePoint.chargePointStatus,info=None),self.loop)
                            time_start = time.time()
